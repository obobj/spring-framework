...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		// 就是处理&符号
		// 因为这个就是FactoryBean和BeanFactory的区别
		final String beanName = transformedBeanName(name);
		Object bean;

		// 这个时候肯定不存在，但是为什么先调用一次，这个时候本来就在初始化
		// 在getBean的时候调用，主要是防止第二次调用
		// 这是其中一个getSingleton(beanName)
		...
		if (sharedInstance != null && args == null) {
			...
			// 如果sharedInstance != null直接拿出来了
			bean = getObjectForBeanInstance(sharedInstance, name, beanName, null);
		}

		else {
			// 这里是表示是原型，如果是原型的化是异常的，但是在最前面已经判断了一遍
			...
			// 没有改造，所以这里基本上为空，只有自己设置parentBeanFactory才运行
	...
	public void addBeanPostProcessor(BeanPostProcessor beanPostProcessor) {
		...
		// TODO 这里很奇怪，是直接删除了，不是去重，是删除了再加入是不是有点麻烦
